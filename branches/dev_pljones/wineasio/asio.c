...
{
    //IWineASIOImpl * (IWineASIOImpl *)iface;
...
//  TRACE("buffer size: %ld\n", This.block_frames);
    ...
    if (!This.input)
    {
        ...
        MESSAGE("Not enough memory for %d input channels\n", This.num_inputs);
        return ASIOFalse;
    }
    ...
    if (!This.output)
    {
        ...
        MESSAGE("Not enough memory for %d output channels\n", This.num_outputs);
        return ASIOFalse;
    }
...
WRAP_THISCALL( ASIOError __stdcall, IWineASIOImpl_start, (LPWINEASIO iface))
{
    ...
    if (This.callbacks)
    {
        ...
        TRACE("inputs desired: %d; JACK outputs: %d\n", This.num_inputs, numports);
        ...
        for (i = j = 0; i < This.num_inputs; i++)
        {
            ...
            if (This.input[i].port)
                ...
                TRACE("Registered input port %i: '%s' (%p)\n", i, This.input[i].port_name, This.input[i].port);
            else {
                //MESSAGE("(%p) Failed to register input port %i ('%s')\n", This, i, This.input[i].port_name);
                MESSAGE("Failed to register input port %i ('%s')\n", i, This.input[i].port_name);
                return ASE_NotPresent;
            }
            ...
            if (j < numports) {
               ...
               TRACE("%d: Connect JACK output '%s' to my input '%s'\n", i
                   ,(envi ? envi : ports[j])
        ...
        TRACE("JACK inputs: %d; outputs desired: %d\n", numports, This.num_outputs);
        ...
        for (i = j = 0; i < This.num_outputs; i++)
        {
            ...
            if (This.output[i].port)
                ...
                TRACE("Registered output port %i: '%s' (%p)\n", i, This.output[i].port_name, This.output[i].port);
            else {
                //MESSAGE("(%p) Failed to register output port %i ('%s')\n", This, i, This.output[i].port_name);
                MESSAGE("Failed to register output port %i ('%s')\n", i, This.output[i].port_name);
                return ASE_NotPresent;
            }
            ...
            if (j < numports) {
               ...
               TRACE("%d: Connect my output '%s' to JACK input '%s'\n", i
